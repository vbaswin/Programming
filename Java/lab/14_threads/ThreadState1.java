...
class ABC implements Runnable {
    public void run() {
        ...
        System.out.println("The state of thread t1 while it invoked the "
                + " method join() on thread t2 - " + ThreadState1.t1.getState());
        try {
            Thread.sleep(200);
        }
        catch (InterruptedException ie) {
...
public class ThreadState1 implements Runnable {
    ...
    public void run() {
        ...
        System.out.println("The state of thread t2 after spawing it - "
                + t2.getState());
        t2.start();
        ...
        System.out.println("Teh state of thread t2 when it has completed"
